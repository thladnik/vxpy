...
class OnlineAnalysisRoutine(vxroutine.WorkerRoutine):
    ...
    def _switch_engine(self):
        if self.current_engine_name not in OnlineAnalysisRoutine_engine_instances.keys():
...
class BaseAnalysisWidget(QtWidgets.QWidget):
    ...
    def __init__(
        ...
        # --- Double-click size toggle ---
        self._normal_height: int = 200
        ...
        self.set_fixed_height(self._normal_height)
...
class TuningCurveWidget(BaseAnalysisWidget):
    ...
    widget_parameters = {
        ...
        "df_f_threshold": 0.2,
        # "max_timepoints": 1000,
        # "regression_update_interval": 100
    ...
    def update_from_data(self) -> None:
        ...
        try:
            ...
            if not len(epoch_indices):
                return
            from_idx = epoch_data[-1]["f0_index"]

            test = safe_read_attribute(f"roi_{self.index}_df/f")
            ...
            print(f"[Tuning Curve Widget]: last index: {test[0]}, last time: {test[1]}, last value: {test[2]}, len of values since last init: {len(left_eye)}, from_index: {from_idx}, shape: {df_f.shape}")

            if len(left_eye) == 0:
                return

            # Keep only the last max_timepoints
            if len(df_f) > self.max_timepoints:
                df_f = df_f[-self.max_timepoints:]
                left_eye = left_eye[-self.max_timepoints:]
                right_eye = right_eye[-self.max_timepoints:]
            ...
            for eye_vals, items in eye_data:
                ...
                self.data_counter += 1
                # Only update regression every nth timepoint
                if self.data_counter % self.regression_update_interval == 0:
    ...
    def _initialize_f0(self, frame: np.ndarray, roi: "BaseROI", roi_idx: int, time: float, frame_index: int):
        ...
        if len(self._f0_temp_save[roi_idx]) >= self.engine_parameters['f0_time_points']:
            ...
            print(
                ...
                f"at frame {self._f0_epoch_start[roi_idx]['frame_index']}, time {self._f0_epoch_start[roi_idx]['time']}, "
    ...
    def _populate_roi_widgets_from_routine(self) -> None:
        ...
        def _try_populate():
            ...
            for index, roi_key in assignments.items():
                ...
                # Set container height to match the widget's normal height
                container.setFixedHeight(analysis_widget._normal_height)
                ...
                roi_label = ClickableLabel(f"ROI {index}")
                font = roi_label.font()
                font.setPointSize(9)
                ...
                container_layout.setAlignment(analysis_widget, QtCore.Qt.AlignTop)

                # Store normal height on container
                container._normal_height = analysis_widget._normal_height
                container._expanded = False

                def toggle_container_height():
                    if container._expanded:
                        container.setFixedHeight(container._normal_height)
                    else:
                        container.setFixedHeight(container._normal_height * 2)
                    container._expanded = not container._expanded

                roi_label.doubleClicked.connect(toggle_container_height)
