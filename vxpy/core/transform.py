...
class Spherical4ScreenCylindricalTransform(BaseTransform):
    ...
    def __init__(self):
        ...
        lower_elevation = -np.pi / 4
        upper_elevation = np.pi / 4

        self._mask_model = sphere.SimpleUVSphere()
        _, mask_positions, mask_faces = self._mask_model.get_subset(-np.pi, np.pi, lower_elevation, upper_elevation)
        self._mask_position_buffer = gloo.VertexBuffer(mask_positions)
        self._mask_index_buffer = gloo.IndexBuffer(mask_faces.flatten())
