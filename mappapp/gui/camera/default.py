...
from mappapp import IPC,Def,Config
import mappapp.routines.camera.Core
...
class FrameStream(AddonWidget):
    ...
    class CameraWidget(QtWidgets.QWidget):
        ...
        def update_frame(self):
            idx, time, frame = IPC.Camera.read(mappapp.routines.camera.Core.Frames,f'{self.device_id}_frame')
...
class EyePositionDetector(AddonWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.detection_mode.addItems(
            [mappapp.routines.camera.Core.EyePositionDetection.from_ellipse.__name__,
             mappapp.routines.camera.Core.EyePositionDetection.feret_diameter.__name__])
    ...
    def update_detection_mode(self,mode):
        IPC.rpc(Def.Process.Camera,
                mappapp.routines.camera.Core.EyePositionDetection.set_detection_mode,
                mode)

    def update_image_threshold(self,threshold):
        IPC.rpc(Def.Process.Camera,
                mappapp.routines.camera.Core.EyePositionDetection.set_threshold,
                threshold)

    def update_particle_minsize(self,minsize):
        IPC.rpc(Def.Process.Camera,
                mappapp.routines.camera.Core.EyePositionDetection.set_min_particle_size,
                minsize)

    def update_sacc_threshold(self,sacc_threshold):
        IPC.rpc(Def.Process.Camera,
                mappapp.routines.camera.Core.EyePositionDetection.set_saccade_threshold,
                sacc_threshold)

    def update_frame(self):
        routine_cls = mappapp.routines.camera.Core.EyePositionDetection
    ...
    class Rect(pg.RectROI):
        ...
        def update_rect(self):
            ...
            IPC.rpc(Def.Process.Camera,mappapp.routines.camera.Core.EyePositionDetection.set_roi,self.id,self.rect)
