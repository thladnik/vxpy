"""
MappApp ./gui/TailDeflectionDetector.py - Custom addons which handle UI and visualization with camera process.
...
class TailDeflectionDetector(QtWidgets.QWidget):
    def __init__(self, parent, **kwargs):
        ...
        self.graphicsWidget = GraphicsWidget(parent=self)
        self.layout().addWidget(self.graphicsWidget, 0, 0)

        self.fps = 30

        self._tmr_frameUpdate = QtCore.QTimer()
        self._tmr_frameUpdate.setInterval(1000 // self.fps)
        self._tmr_frameUpdate.timeout.connect(self.updateFrame)
        self._tmr_frameUpdate.start()

    def updateFrame(self):
        self.graphicsWidget.imageItem.setImage(np.rot90(IPC.BufferObject.readBuffer('TailDeflectionDetector'), -1))
...
class GraphicsWidget(pg.GraphicsLayoutWidget):
    def __init__(self, **kwargs):
        ...
        self.fishMarkerRects: dict = IPC.BufferObject._buffers[self.parent().__class__.__name__].fishMarkerRects
        self.extractedRects: dict = IPC.BufferObject._buffers[self.parent().__class__.__name__].extractedRects
        self.tailDeflectionAngles = IPC.BufferObject._buffers[self.parent().__class__.__name__].tailDeflectionAngles
    ...
    def mouseClicked(self, ev):
        ...
        ### Second click: end line and create rectangular ROI + subplot
            ...
            ## Draw line
            lineSegROI = Line(self, self.currentId, self.newMarker,
            ...
            rectROI = Rect(self, self.currentId, self.newMarker)
